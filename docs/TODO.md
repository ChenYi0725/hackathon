...
狀態使用「待認領／進行中／待審查／完成／受阻」。認領與協調以 open PR 為準；開發者在自己的分支更新對應列的負責人與 PR，避免只改本地 TODO 就假定其他人已看到。以下任務尚未有人認領。

| ID | 優先序 | 工作 | 負責區塊 | 前置 | 狀態 | 負責人／PR |
| --- | --- | --- | --- | --- | --- | --- |
| DDD-00 | P0 | 共用資料與 port 契約定案 | Application＋Domain | 無 | 待認領 | — |
...
預計新增 `docs/contracts.md`，由後續實作共同遵循；這張任務先完成設計文件，不以未接線的空介面宣稱功能完成。
