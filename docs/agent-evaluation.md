...
修正版已於 2026-09-13 更新競賽 EC2，使用既有部署底版加本修正（部署 commit
`7b1ac57c98ffa8d98651be491d2a379e4ef93ccd`）。更新前停止服務並備份完整資料目錄；
重啟健康檢查成功。對比更新前後資料列雜湊，4 案件、14 修訂、3 基準、5 文件與
3 筆來源綁定完全一致；未重新 OCR 或回填歷史案件。

線上可見 Chrome 再測兩個原問題皆 HTTP 200、規則與程式審查成功；第一題也成功讀取
來源頁。但兩題最終皆為 `insufficient_evidence`：第一題仍有無效生成引用，第二題僅取規則
與計算，未取文件引用。介面保留 11 項通過的確定性檢核並提示來源不足，沒有假裝生成
有根據的答案。查詢後再次比對上述資料列雜湊，案件、修訂與來源仍完全相同。
