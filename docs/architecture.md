...
3. 啟動獨立子程序，以 PDFium 逐頁抽取可用文字層與座標；不符合條件的頁面才轉圖片並由 PaddleOCR／RapidOCR 在 CPU 辨識。只有遇到需要 OCR 的頁面才初始化模型。
4. 保存文字框、信心值、像素座標及頁面尺寸，再按位置重建供解析器使用的文字。
5. 已知版型轉為待確認草稿；原文、PDF 與案件分別保存。

子程序不接受模型工具指令，也不呼叫雲端文件處理服務。所選引擎的模型首次從官方來源下載。逾時會終止子程序；文字層判定發生在 OCR 之前，OCR 失敗仍明確回報失敗。文字頁標記為 `pdf-text`，掃描頁保留實際 OCR 引擎名稱；混合文件可有不同逐頁來源。詳見 [文字層加速](pdf-text-fast-path.md)。
...
5. 呼叫 Bedrock Converse，欄位抽取輸出上限為 8192 tokens。SDK 自動重試關閉，每次請求最多三次 adapter 嘗試，皆經過節流。如果模型因長度截斷且因素超過 12 項，改成每批最多 12 個因素；每批仍使用完整原文與相同頁碼、行號，共用原有鎖與節流。所有批次完整成功後才合併驗證，任何批次失敗都不保存部分草稿。
