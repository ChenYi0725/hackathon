...
同案 PDF／Excel、最多三筆比較標的、固定基準發布、獨立程式檢核、人工接受／拒絕，以及後端 PDF／Excel／ZIP 匯出已接通。完整實作對照、可重跑 Demo、測試及未支援範圍見 [核心流程驗收](docs/core-workflow.md)。表 3／4／5 Excel 模板及 PDF 下載已整合；完整樹林住宅公式與原始 PDF 套印仍不可視為完成。

```mermaid
flowchart TD
    R["人工建立規則草稿／上傳準則原文"] --> P["核對原文、矩陣、適用期間<br/>發布不可覆寫的固定版本"]
    P --> C["建立案件並明確綁定版本<br/>比準地、最多三比較標的、區段、日期"]
    C --> D["同案上傳 PDF／Excel<br/>保留原始文件及新版本"]
    D --> O["PaddleOCR／Excel 內容解析<br/>原值、頁碼、儲存格、公式資訊"]
    O --> H["人工修正、儲存、再次確認"]
    H --> A["查核計畫／選用 Bedrock Agent<br/>使用固定白名單工具"]
    A --> K["依案件版本與適用範圍檢索原文"]
    A --> E["選用官方公園查證／GIS 量測<br/>分開保存原值與佐證，失敗不當作不存在"]
    H --> V["程式查級距、矩陣、加總、跨表<br/>未確認／未支援不得通過"]
    K --> V
    E --> V
    V --> S["檢核快照<br/>綁定輸入、規則、證據版本"]
    S --> M["人工接受或拒絕，記錄原因與時間<br/>不改寫技術判定"]
    M -->|"修改後舊結果立即失效"| H
    M --> X["驗證快照仍有效<br/>下載 PDF、Excel 整理書表、ZIP"]
```

Agent 外部查詢是未保存預覽；正式佐證由案件「外部資料與 GIS」操作保存。找不到適用文件、沒有幾何或歷史時點、API 失敗時保留資料不足／待確認。Excel 公式只讀原式及快取，不自動執行；採用儲存格後仍須人工確認。新版發布不自動改變既有案件。逐欄引用固定原文件、頁碼或儲存格；追加 PDF 與採用 AI 草稿不混用舊來源。檢核也綁定引擎版本，舊引擎快照須重新檢核後才能匯出。
...
flowchart TB
    ...
    RESULT -->|繼續修正| HUMAN
    RESULT --> EXPORT["匯出 JSON、CSV、HTML<br/>或以有效快照下載 PDF、Excel、ZIP"]
```

載入案件時也會計算目前審查狀態；未確認資料不會自動通過。儲存時若 revision 已過期，回傳衝突並要求重新載入。尚有疑點的案件仍可匯出，報告會保留待確認狀態。後端 PDF、Excel 整理書表及多比較標的已由上述核心流程接通；官方套印、完整住宅規則及 GraphRAG 仍未完成。
...
| `PDF_FONT_PATH` | 系統繁中字型 | 指定可嵌入的繁體中文 TTF，後端 PDF 必需 |
| `APP_ACCESS_TOKEN` | 未設定 | 私有 API 存取認證；未設定時只接受本機 |
