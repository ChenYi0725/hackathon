...
1. 在案件工作台或評價基準庫按「上傳評價基準表」，填寫與 PDF 表格標題一致的縣市行政區，選擇 PDF（上限 20 MB），按「開始 OCR 與規則轉換」。適用期間可留到下一步填寫。
2. 辨識完成後，按「開啟上傳的基準 PDF 核對原文」，核對候選基準、因素與警告。依來源填入適用起日、迄日（含），並依矩陣表頭選擇列／欄方向；不知道期間或方向時先查明，不任意填寫。
3. 勾選人工確認，按「建立基準並加入檢索」。原始 PDF 會與新版本一起保存，作為該版本的檢索依據。
4. 接著在「上傳題目並開始填表」確認所選基準、選擇題目 PDF。核對案件資料、因素與引用，儲存後逐項確認，再從「匯出成果」下載完整審查 Excel。

這份表提供各因素的級距與修正率矩陣，供案件審查查表；它與待審查的題目 PDF 是不同文件。OCR 失敗時，對話框會顯示原因並保留檔案與欄位，可修正地區、換檔或重試。逾時可拆分文件；若提示找不到標題或完整矩陣，請核對地區文字與表格版型，系統不會補造規則。
