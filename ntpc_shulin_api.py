...
def search_ntpc_datasets(
    ...
    result = [
        item for item in _swagger_datasets(unit_id, timeout)
...
def _fetch_schools(source: dict, school_year: int, timeout: float,
                   matcher: DistrictMatcher) -> list[dict]:
    url = source["url"]
    if source["format"] == "csv":
        rows = list(csv.DictReader(io.StringIO(_read_url(url, timeout))))
    else:
...
def fetch_valuation_factors(
    ...
    if wanted is None or "公共建設" in wanted:
        for source in _school_sources(school_year):
            if wanted_keys is not None and source["key"] not in wanted_keys:
                continue
            total_sources += 1
            slot = bucket("公共建設", "接近學校之程度")
            metadata = {
                "key": source["key"], "name": source["name"], "url": source["url"],
                "factor": "公共建設", "item": "接近學校之程度",
                "period": f"{school_year} 學年度", "status": "ok", "matched_count": None,
            }
            try:
                records = _fetch_schools(source, school_year, timeout, matcher)
            except (RuntimeError, ValueError, csv.Error) as exc:
                if strict:
                    raise
                metadata["status"] = "error"
                metadata["error"] = str(exc)
                report["errors"].append({"source": source["key"], "message": str(exc)})
            else:
    ...
    for source in NTPC_SOURCES:
        ...
        if source["heavy"] and not include_heavy:
            ...
            continue
        try:
            rows = fetch_ntpc_dataset(
                source["dataset_id"], page_size=1000, max_pages=200, timeout=timeout)
            _validate_rows(rows, url, (source["name_keys"][0],))
            firm, loose = [], []
            for row in rows:
                basis = matcher.match(row, source)
                if not basis:
                    continue
                record = _build_record(row, source, url, basis, matcher)
                if basis == "name_keyword":
                    loose.append(record)
                else:
                    firm.append(record)
                    if record["longitude"] is not None:
                        strict_points.append((record["longitude"], record["latitude"]))
...
def fetch_public_facilities(
    ...
    return fetch_valuation_factors(
        ...
        only_factors=("公共建設",), only_source_keys=keep)
