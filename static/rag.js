...
export function setupRag({state,api,modal,esc,save}) {
 document.addEventListener('click',async event=>{
  ...
  modal('依據問答',`<p>基準：${esc(rules.name)} / ${esc(rules.version)}。只檢索相同地區、用地與適用期間的文件；案件日期需為 YYYY-MM-DD 或民國 YYYMMDD。</p>
   ...
   <p><label><input type="checkbox" id="rag-consent">我已確認問題與來源文件符合上雲規範，不含個資或財務資訊。</label></p>
   <div class="actions"><button id="rag-search">本機查找來源</button><button id="rag-answer">AWS 生成說明</button></div>
  ...
  async function query(generate){
   ...
   output.textContent=generate?'正在檢索並生成說明…':'正在本機查找來源…';
   const result=await api('/api/cases/'+currentCase.id+'/evidence',{method:'POST',body:JSON.stringify({revision:currentCase.revision,question,generate,cloud_data_approved:approved})});
   const numbers=new Map(result.hits.map((h,i)=>[h.id,i+1]));
   output.innerHTML=`<p>${esc(result.message)}</p>${result.status==='no_evidence'?'<p>找不到符合版本、日期與問題的來源。</p>':''}${result.status==='insufficient_evidence'?'<p>現有原文不足以回答，請人工核對或補充文件。</p>':''}
